...
{
	//显示当前曲目的索引,及总曲目数
	OLED_ShowNum(0,51,index,3,12);		//索引
	OLED_ShowChar(19,51,'/',12,1);
	OLED_ShowNum(25,51,total,3,12); 	//总曲目
    OLED_Refresh_Gram();	
}
//显示当前音量
...
//	OLED_ShowString(30+110,48,"VOL:",12);	  	  
	OLED_ShowNum(100,51,vol,2,12); 	//显示音量
	OLED_Refresh_Gram();
...
{	
	...
	else if((time!=playtime)&&(time!=0))//1s时间到,更新显示数据
	{
		...
		//显示播放时间			 
		OLED_ShowNum(0,36,time/60,1,12);		//分钟
		OLED_ShowChar(6,36,':',12,1);
		OLED_ShowNum(12,36,time%60,2,12);	//秒钟		
 		OLED_ShowChar(24,36,'/',12,1); 	    	
   // OLED_Refresh_Gram();			
		//显示总时间
		if(f_kbps)time=(lenth/f_kbps)/125;//得到秒钟数   (文件长度(字节)/(1000/8)/比特率=持续秒钟数    	  
		else time=0;//非法位率	  
 		OLED_ShowNum(30,36,time/60,1,12);	//分钟
		OLED_ShowChar(36,36,':',12,1);
		OLED_ShowNum(42,36,time%60,2,12);	//秒钟	
  //  OLED_Refresh_Gram();			
		//显示位率			   
   		OLED_ShowNum(84,36,f_kbps,3,12); 	//显示位率	 
...
void mp3_play(void)
{
	...
	u16 *mp3indextbl;	//音乐索引表 
 	...
 	{	    
		Show_Str(0,16,127,16,"文件夹错误!",16,0);
		OLED_Refresh_Gram();
		delay_ms(200);				  
		OLED_Fill(0,16,127,63,0);//清除显示	     
		delay_ms(200);				  
	} 									  
	totmp3num=mp3_get_tnum("0:/MUSIC"); //得到总有效文件数
  	while(totmp3num==NULL)//音乐文件总数为0		
 	{	    
		Show_Str(0,16,127,16,"没有音乐文件!",16,0);
		OLED_Refresh_Gram();
		delay_ms(200);				  
		OLED_Fill(0,16,127,63,0);//清除显示	     
		delay_ms(200);				  
	...
	mp3fileinfo.lfname=mymalloc(SRAMIN,mp3fileinfo.lfsize);	//为长文件缓存区分配内存
 	...
 	{	    
		Show_Str(0,16,127,16,"内存分配失败!",16,0);
		OLED_Refresh_Gram();
		delay_ms(200);				  
		OLED_Fill(0,16,127,63,0);//清除显示	     
		delay_ms(200);				  
	...
	{	
		...
		strcat((char*)pname,(const char*)fn);  			//将文件名接在后面
 		OLED_Fill(0,16,127,32,0);				//清除之前的显示
		
		
		Show_Str(0,16,127,16,fn,16,1);				//显示歌曲名字 
		
		OLED_ShowNum(100,51,(vsset.mvol-100)/5,2,12); 	//显示音量
